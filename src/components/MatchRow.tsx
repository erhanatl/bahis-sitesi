...
export default function MatchRow({ match }: MatchRowProps) {
  ...
  function getPct(oddsA: string | undefined, oddsB: string | undefined): number {
    if (oddsA && oddsB && oddsA !== '-' && oddsB !== '-') return oddsToNormalizedPercent(oddsA, oddsB);
    return 0;
  }

  const pctOver25 = getPct(odds?.overUnder25.over, odds?.overUnder25.under);
  const pctOver35 = getPct(odds?.overUnder35.over, odds?.overUnder35.under);
  const pctBtts = getPct(odds?.btts.yes, odds?.btts.no);
  const pctCorners = getPct(odds?.corners95.over, odds?.corners95.under);
  const pctFhBtts = getPct(odds?.fhBtts.yes, odds?.fhBtts.no);
  const pctFhOver15 = getPct(odds?.fhOverUnder15.over, odds?.fhOverUnder15.under);
